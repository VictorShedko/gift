...
public enum ErrorMassageByException {
    ...
    GENERAL_RUNTIME(1, RuntimeException.class, ex -> {
        return "Unknown runtime exception";
    }, HttpStatus.INTERNAL_SERVER_ERROR),
    GENERAL_APP(2, GiftException.class, ex -> {
        return "Unknown gift exception";
    }, HttpStatus.I_AM_A_TEAPOT),
    UNIQ_FIELD(3, UniqFieldException.class, ex -> {
        return "insert error, violation of field uniqueness. Uniq fields:" + ((UniqFieldException) ex).getUniqFields();
    }, HttpStatus.BAD_REQUEST),
    RESOURCE_NOT_FOUND(4, ResourceNotFoundedException.class, ex -> {
    ...
    ID_IN_INSERT(5, IdInInsertException.class, ex -> {
    ...
    public static JSONExceptionEntity createJSONExceptionEntity(Exception exception) {
        for (var errorMessage : ErrorMassageByException.values()) {
            if (errorMessage.getExceptionClass() == exception.getClass()) {
                return new JSONExceptionEntity(errorMessage.code,
                        errorMessage.messageBuilder.apply(exception),errorMessage.status );
            }
        }
        if(exception instanceof RuntimeException){
            return new JSONExceptionEntity(GENERAL_RUNTIME.code,
                    GENERAL_RUNTIME.messageBuilder.apply(exception),GENERAL_RUNTIME.status);
        }
        return new  JSONExceptionEntity(GENERAL.code,
                GENERAL.messageBuilder.apply(exception), GENERAL.status );
